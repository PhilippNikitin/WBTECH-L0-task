...
func main() {
	...
	if err != nil {
		logging.Logger.Fatalf("Ошибка при подключении к NATS streaming серверу: %v", err)
	}
	defer sc.Close()

	/*В окончательной версии отправлять 1000 заказов циклом*/
	// Публикация заказа в Nats-streaming
	for i := 0; i < 11; i++ {
		data, err := json.Marshal(makeFakeOrder())
		if err != nil {
			logging.Logger.Fatalf("Ошибка при сериализации данных: %v", err)
		}

		err = sc.Publish("orders", data)
		if err != nil {
			logging.Logger.Fatalf("Failed to publish order: %v", err)
		}

		fmt.Println("Тестовый заказ был отправлен в Nats-streaming")
		logging.Logger.Println("Тестовый заказ был отправлен в Nats-streaming")
	}
}
